...
- 艦船建造画面（ship-builder.html）に、上記関数の計算結果を実際の
  ビューポート上に半透明のワイヤーフレーム箱として表示する確認用の
  デバッグ機能を追加（「当たり判定確認」ボタン、ビューポート右上）。
  本実装（保存データへの組み込み）とは別の、開発中の目視確認専用の
  一時的な機能。モデルの回転・スケール・オフセット調整のたびに
  自動で再計算・再描画される。
