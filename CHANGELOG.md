...
### 修正: プレイ画面のパンが上下左右すべて逆だった不具合

- **バグ報告**: v75適用後も、プレイ画面のパンだけ上下左右すべてが
  逆に動く（艦船建造画面は正しく動く、と確認済み）。
- **原因**: `08-camera.js`の`_applyScreenPan`と、`09-ship-builder.js`
  ・`12-port-builder.js`の`applyScreenPan`は、どちらも同じ形の式
  `worldDelta = right*dx - up*dy`で注視点を動かしていたが、実際に
  カメラ行列（`camera.matrixWorld`）から直接求めた「真の画面右・
  画面上方向」への投影量で両者を同一条件で比較したところ、
  **符号が正反対**になっていることが判明した。09/12は「右ドラッグで
  景色が右についてくる」向きに、08は「右ドラッグでカメラ自体が
  右へパンする（景色は左へ流れる）」向きになっていた。
  これまでの調査では実装が違う2ファイル同士を毎回別々の検証条件で
  比較していたため、この符号の食い違いを見逃し、無関係な部分（
  view/upベクトルをローカル座標かワールド座標かで計算する処理）を
  何度も修正しては撤回していた。
- **修正**: `08-camera.js`の`worldDelta`全体の符号を反転し、
  09/12ファイルと完全に同じ向き（右ドラッグで景色が右へ、上ドラッグ
  で景色が上へついてくる）に統一した。船の姿勢（回転）を0度・45度・
  90度・180度・270度・-60度で変えながら、カメラ行列から直接求めた
  画面右・画面上方向への投影で09/12方式と一致することを検証済み。

---

## v76 (開発中) - 逆走進入防止機能（進行中、当たり判定確認機能を追加）

### 新機能（実装中）: ドッキングポートへの逆走進入防止

大きめの新機能のため段階的に実装中。現時点では以下が完了:

- `01-state-and-config.js`に`computeSegmentedBoundsFromObject3D()`を
  追加。艦・ポートのモデル形状を、ローカルZ軸方向に8区間へ分割し、
  各区間ごとの幅・高さを頂点走査から求める「大雑把な区間分割
  バウンディングボックス」を計算する関数。細いロケット型・太い戦艦型
  といった見た目の違いをある程度反映した衝突判定用の形状マスクを
  安く得るのが目的（今後、逆走進入時の物理的ブロック判定に使用予定）。
  Three.js実測で、ロケット状の形状（先端が細く本体が太い）に対して
  正しく段階的な幅・高さが得られること、モデルがワールド上で移動・
  回転していても結果が変わらないことを検証済み。
- 続けて`computeCrossSegmentedBoundsFromObject3D()`を追加。Z軸単独
  では捉えられない左右非対称な形状（片側に張り出したウイング等）を
  補うため、Z軸8分割に加えてX軸（Y軸周りに90度回した軸、艦の左右
  方向）でも8分割し、その交差（格子セル）に実際に頂点が存在する
  ものだけを箱として残す方式（最大64個、頂点の無いセルは単純に
  除外）。Three.js実測で、左右非対称な形状（片側だけにウイングを
  持つテスト形状）に対して、そのウイング部分だけを捉えたセルが
  正しく生成されること、平面的なモデルでの軸の扁退処理、ワールド
  変換への非依存性を検証済み。
- 上記関数をさらに改良し、Z軸・X軸のうち実際に長い方（全体のスパン
  が大きい方）を自動判定して16分割、短い方は8分割のまま、という
  非対称な分割数にした。全長より幅の方が大きいモデルや、元モデルの
  向きが90度ズレているモデルなど、どちらの軸が実際の「長辺」に
  なるかはモデルによって変わるため、固定でZ軸だけ細かく分割すると
  実際に長い方がX軸だった場合に粗いままになってしまう問題への対応。
  スパンの差がわずかな場合はどちらを選んでも体感差はほぼ無いという
  前提で、特別な閾値は設けずスパンの大小比較のみで決定する。
  Three.js実測で、通常の細長い艦（Z軸が長い）・元モデルが90度
  ズレている場合（X軸が長い）・全長より幅が大きい場合（X軸が長い）・
  ほぼ同じ幅の場合の4パターンを検証済み。デバッグ表示の色分けも、
  実際に長い方の軸に沿ってグラデーションするよう合わせて調整。
- 艦船建造画面（ship-builder.html）に、上記2つの関数の計算結果を
  実際のビューポート上に半透明のワイヤーフレーム箱として表示する
  確認用のデバッグ機能を追加（「当たり判定(Z軸)」「当たり判定
  (Z×X軸)」の2つのボタン、ビューポート右上、排他的にどちらか一方
  のみ表示）。本実装（保存データへの組み込み）とは別の、開発中の
  目視確認専用の一時的な機能。モデルの回転・スケール・オフセット
  調整のたびに自動で再計算・再描画される。

### 今後の実装予定
1. 艦・ポートのモデル保存時に区間分割ボックスを事前計算し保存データに
   含める（Z軸単独版・交差版のどちらを採用するかは要検討）
2. `10-docking-platform.js`を複数ゲート対応（`_gates`配列管理）へ拡張
3. `03-thruster-solver.js`に新パラメータ`REVERSE_BLOCK_RADIUS`
   （ポートごとの逆走進入禁止半径）とアプローチ距離の独立設定を追加。
   球内かつ進入軸の奥側（逆走）から接近した場合、警告＋境界+50地点で
   止まれるよう手動・自動を問わず常時強制減速
4. 艦・ポートの区間分割ボックス同士での衝突判定、貫通防止の物理ブロック
5. HUDへの警告表示
6. port-builder.htmlへの設定UI追加
