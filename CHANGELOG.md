...
### 修正: プレイ画面のパンが（船が回転すると）逆になる不具合

- **バグ報告**: v74適用後、船体建造・ポート設定画面のパンは直った
  が、プレイ画面（自由視点モード）のパンだけ完全に逆に感じる。
- **原因**: `08-camera.js`の`_applyScreenPan`は、船が自機である
  ことを踏まえ、画面右方向を求める際に船のローカルupを船の姿勢
  クォータニオンでワールド化した`worldUp`を使っていたが、視線方向
  （`view`）はローカル座標のまま外積に使っていた。ワールド座標の
  ベクトルとローカル座標のベクトルを混ぜて外積を取っていたため、
  船が無回転（プレイ開始直後など）のときだけ偶然正しい結果になり、
  船を操縦して向きが変わるとパンの向きが破綻していた（Three.jsで
  実際にcamera.project()して検証し、無回転時は問題なし・回転時は
  パンが効かない/意図しない方向になることを確認した）。
  `09-ship-builder.js`・`12-port-builder.js`には船の姿勢という概念
  自体が無いためこの問題が起きず、「建造・ポート設定は直ったのに
  プレイ画面だけ逆」という報告と一致する。
- **修正**: `view`も`worldUp`と同じくship.quaternionでワールド化
  してから外積を取るよう統一。あわせて、修正後のパンの向きが
  `09-ship-builder.js`・`12-port-builder.js`（ユーザー確認済みで
  正しい実装）と完全に一致することを、複数の回転角度でThree.js実測
  により検証した。
...
### 修正: パンの左右がまだ逆／視点リセットボタン追加／6方向平行投影ビュー追加

- **バグ報告**: v73でパンの向き反転を試みたが、プレイ画面・船体建造・
  ポート設定のいずれも依然として左右だけが逆に動く（上下は正しい）。
  また視点リセットボタンが欲しい。船体建造・ポート設定には、ブレン
  ダーのように正面/側面/上面ボタンで平行投影の6方向固定視点に切り
  替えられる機能が欲しい（同じボタンをもう一度押すと反対面を表示）。
- **根本原因**: `_applyScreenPan`（`08-camera.js`）および
  `computeScreenAxes`（`09-ship-builder.js`・`12-port-builder.js`）が、
  画面右方向ベクトルの計算に「船（注視点）→カメラ」方向をそのまま
  使っていた。実際にカメラが見ている向き（カメラ→船）はこの逆な
  ので、右方向ベクトルの符号がちょうど反転していた。上方向ベクトル
  はこの符号反転が計算過程で2回噛み合って偶然相殺されるため、上下
  だけは正しく見えていた。3ファイルとも、右方向・上方向の計算元を
  「カメラ→船」方向（view方向）に修正。
- **視点リセットボタン**: `08-camera.js`に`CameraSystem.resetView()`
  を新設し、角度・ズーム（距離）・パンずれを一括で初期状態へ戻す
  ようにした（既存の前後左右上下プリセットは角度とパンのみのリセ
  ットで、ズームはそのままだったため）。プレイ画面のHUD視点パネル
  （`06-hud.js`）にボタンを追加。船体建造・ポート設定画面にも同様の
  「リセット」ボタンを追加。
- **6方向平行投影ビュー**: `09-ship-builder.js`・`12-port-builder.js`
  に本物の`THREE.OrthographicCamera`を追加し、既存の透視投影カメラ
  と切り替えられるようにした。ビューポート右上に「正面」「側面」
  「上面」ボタンを配置し、同じボタンをもう一度押すと反対面（背面／
  反対側の側面／下面）へ切り替わる（ブレンダーのテンキー視点と同様
  の挙動）。平行投影表示中に自由にドラッグ回転すると、面に正対しな
  くなるため自動的に通常の透視投影へ戻る。ズーム（ホイール／ピンチ）
  は平行投影時、正しくフラスタムサイズ（表示範囲の広さ）に反映され
  るようにした。
